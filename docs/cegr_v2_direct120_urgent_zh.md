# CEGR V2 紧急 2+120 路线操作手册

> 适用场景：CEGR V1 和课程 baseline 已经完成，现在时间紧，希望先做 2 步冒烟，再直接训练一个 120 步改进模型。
>
...
4. 单臂不是算法要求。它只是复用已有 baseline，从而少训练一个 120 步模型的紧急省时方案。
5. 这条路线比较的是“旧 Search-R1 baseline”与“分组修复 + EFF”的整体差异，**不能单独归因**为 F1 fallback 的效果。

若必须严格回答“提升来自分组修复还是 F1 fallback”，需要再训练一个从原始 Qwen 起跑的 `Grouped-EM-120` 对照。项目原有的 `Grouped-EM-40` 与 `EFF-40` 双臂路线更适合做该因果拆分；本手册不替换那条标准路线。
...
紧急路线省掉了第二个新训练臂和训练中的周期性 700 题验证，因此是“关键训练预算匹配”，不是全流程计算量完全相同的随机对照试验。分析文件会自动记录这个限制。
...
任一 `test` 失败都先停止。应从已保存的数据盘或证据备份恢复 V1，不要用紧急脚本临时重训 baseline。
...
`prepare_experiment.sh` 会检查 V1 哈希、生成不重叠 pilot、运行全部单元测试、编译 Python、检查所有 shell 语法，并确认 V2 没有修改 `search_r1/` 或 `verl/`。
...
cd "$V2_ROOT"
bash scripts/improvement_v2/run_direct120_smoke.sh
```
...
cd "$V2_ROOT"
bash scripts/improvement_v2/run_direct120.sh
```
...
cd "$V2_ROOT"
bash scripts/improvement_v2/evaluate_direct120.sh
```
...
```text
statistically_supported_direct120_improvement
  可以写“在本实验设置下获得统计支持的整体改进”。

directional_direct120_improvement
