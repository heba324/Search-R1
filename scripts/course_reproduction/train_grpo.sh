...
RUN_NAME="${RUN_NAME:-search-r1-course-qwen2.5-1.5b-grpo-bm25}"
ARTIFACT_DIR="$REPO_ROOT/artifacts/course-reproduction/$RUN_NAME"
...
printf 'status=completed\nmodel=Qwen/Qwen2.5-1.5B-Instruct\nalgorithm=grpo\nretriever=bm25\ntraining_steps=%s\ntrain_batch_size=%s\ngroup_size=%s\nmax_turns=4\ntopk=3\nelapsed_seconds=%s\n' \
  "$TOTAL_STEPS" "$TRAIN_BATCH_SIZE" "$GROUP_SIZE" "$ELAPSED" > "$MARKER.tmp"
